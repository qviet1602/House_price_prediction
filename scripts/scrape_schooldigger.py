...
def scrape_schooldigger():
    ...
    schools_csv_file.write("State,City,Zipcode,County,Address,School Name,Average Standard Score,SchoolDigger Rating")
    ...
    try:
        ...
        for i in range(total):
            ...
            url = "https://www.schooldigger.com/go/" + info.get_state() + "/zip/" + info.get_zipcode() + "/search.aspx"
            print("URL: ", url, " i:", i)
            ...
            if html_page.status_code == 200:
                ...
                for each_row in tbody_data_children:
                    if each_row != '\n':
                        ...
                        average_standard_value = parse_average_standard_score(average_standard_column)
                        schooldigger_rating_value = parse_schooldigger_rating(schooldigger_rating_column)

                        schools_csv_file.write(info.get_state() + "," + city_value + "," + info.get_zipcode() + "," + county_value + "," + address_value + "," + school_name + "," + average_standard_value + "," + str(schooldigger_rating_value))
                        schools_csv_file.write("\n")
            else:
                print("Error ")
    finally:
