def condo():
    ...
    month_year_list = []
    with open(
        "data/ZHVI/condoCoOpTimeSeries/City_Zhvi_Condominum.csv", "r"
    ) as csv_file:
        line_count = 0
        for line in csv_file:
            ...
            for idx in range(6, len(line_parts), 1):
                if line_parts[idx].strip('"') != "":
                    insert_line = (
                        "INSERT INTO city_timeseries (city_id, home_type_id, year_month, zillow_id, index_value)"
                        + " VALUES("
                        + city_select
                        + ","
                        + home_type_select
...
def fourBedroom():
    ...
    month_year_list = []
    with open(
        "data/ZHVI/fourBedroomTimeSeries/City_Zhvi_4bedroom.csv", "r"
    ) as csv_file:
        line_count = 0
        for line in csv_file:
            line = line.rstrip("\n")
...
def fivePlusBedroom():
    ...
    with open(
        "data/ZHVI/fivePlusBedroomTimeSeries/City_Zhvi_5BedroomOrMore.csv", "r"
    ) as csv_file:
        line_count = 0
        for line in csv_file:
            line = line.rstrip("\n")
