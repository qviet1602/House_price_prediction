def process_data(input_file_name, feature, output_file_name):
    ...
    with open(input_file_name, "r", encoding="latin-1") as csv_file:
        ...
        for line in csv_file:
            ...
            city_select = (
                "(select city.id from city inner join county on city.county_id = county.id "
                "inner join state on county.state_id = state.id where county.name ='"
                + county_name
            ...
            for idx in range(5, len(line_parts), 1):
                if line_parts[idx].strip('"') != "":
                    ...
                    insert_line = (
                        "INSERT INTO city_median_price (city_id, home_type_id, year_month, list_price)"
                        + " VALUES("
                        + city_select
                        + ","
                        + home_type_select
                        + ",'"
                        + month_year_list[idx - 5].strip('"')
                        + "','"
                        + str(list_price)
                        + "');"
                    )
