...
def scrape_bestplaces_income():
    ...
    try:
        ...
        for i in range(17728, total, 1):
            ...
            url = "https://www.bestplaces.net/economy/zip-code/" + state + "/" + info.get_city() + "/" + info.get_zipcode()
                ...
                if is_oops:
                    ...
                    for each_split_value in split_values:
                        if "average income" in each_split_value:
                            average_value_idx = each_split_value.find('resident is $',0, len(each_split_value))
                            if average_value_idx != -1:
                                average_value = each_split_value[average_value_idx + 13:]
                                average_value = average_value.replace('a year', '')
                                average_value = average_value.replace(',', '')
                        if "Median household" in each_split_value:
                            median_value_idx = each_split_value.find('resident is $', 0, len(each_split_value))
